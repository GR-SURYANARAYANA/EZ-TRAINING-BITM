class IA:
  ...
  def __cal_grade(self):
    ...
    score = self.__percentage // 10
    return grade[score] if score in grade else "Invalid Marks type"
  def __print_marks(self):
    str = [f"Subject {i+1} : {self.__IA[i]}" for i in range(len(self.__IA))]
    return "\n".join(str)

  def __str__(self):
    return str.format(f"{self.usn},{self.name},{self.__IA}")
